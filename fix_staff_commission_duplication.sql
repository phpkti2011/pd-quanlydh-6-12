-- =====================================================
-- Fix Staff Commission 4.0: DUPLICATION FIX
-- Issue: Nhân viên vừa làm SX vừa làm Quản lý bị xuất hiện 2 lần (UNION ALL).
-- Fix: Dùng CTE để UNION rồi GROUP BY participant_name.
-- Logic tính tiền vẫn giữ nguyên (Main/1000, Sub/100).
-- =====================================================
...
BEGIN
    ...
    SELECT 
        ...
        SUM(p_main) as main_task_comm,
        SUM(p_sub) as sub_task_comm,
        SUM(p_main + p_sub) as total_comm
    FROM raw_data
    GROUP BY p_name;
