-- ==============================================================================
-- FIX SALES COMMISSION: Personal Tier + Group Target Logic
-- ==============================================================================
...
BEGIN
    ...
    WITH sales_data AS (
        ...
        GROUP BY p.id, p.full_name, p.commission_tiers, p.commission_sales, p.role
    ),
    ...
    group_info AS (
        SELECT 
            ...
            -- We assume 'Sales' department target is the "Group KPI"
            COALESCE((
                SELECT target_amount FROM sales_targets
    ...
    SELECT 
        ...
        -- Group Bonus Logic:
        -- 1. Must reach GROUP TARGET (from group_info)
        -- 2. Must be ADMIN role (or specifically allowed) - checking 'Admin' role here based on user feedback "cho Sales Ad"
        -- 3. Apply group_rate to grand_total
        CASE 
