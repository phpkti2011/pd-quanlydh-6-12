-- ==============================================================================
-- FIX: Stage Commission Rate Lookup Logic
-- Issue: When commission_stages doesn't have a key (e.g., 'In'), SQL falls back
--        to global policy rate instead of treating it as 0.
-- ==============================================================================

-- Option 1: Update calculate_staff_commission function
-- Change the rate lookup from COALESCE to CASE to properly handle missing keys
...
BEGIN
    ...
    WITH participated_tasks AS (
        SELECT 
            ...
            -- FIX: Use CASE to check if key exists in JSONB
            -- If key exists, use that value (even if 0)
            -- If key doesn't exist, check if ALL keys are empty (new employee) -> use global
            -- Otherwise -> use 0 (employee has been configured but this stage is not set)
            CASE 
...
-- Also update get_staff_activity_details function
