-- =====================================================
-- Fix Staff Commission 3.0: MIXED RATES FIX
-- 1. Hoa hồng Quy trình (Process/Main): Chia 1000 (Để fix lỗi gấp 10 lần)
-- 2. Hoa hồng Công đoạn (Subtask/Stage): Chia 100 (Giữ nguyên tỉ lệ %)
-- 3. Vẫn loại trừ NVKD và giữ nguyên logic Quản lý
-- =====================================================
...
BEGIN
    ...
    staff_calc AS (
        SELECT 
            ...
            -- Process Comm (Quy trình): DIVIDE BY 1000 (Fixing 10x error for main tasks)
            SUM(CASE 
            ...
            -- Sub-task Comm (Công đoạn): DIVIDE BY 100 (Standard Percentage)
            SUM(CASE 
                WHEN pt.stage_value > 0 THEN 
