-- Update get_staff_activity_details to include Product Manager's sales-based commission details
...
BEGIN
    RETURN QUERY
    -- 1. Regular Staff Commission Details (Existing Logic)
    ...
    SELECT
        p.full_name,
        o.order_code,
        'DoanhSo' as stage, -- Special stage name for UI display
        o.created_at as started_at,
        o.delivery_date as finished_at,
        1.0 as score, -- Default score
        1 as participant_count,
        ROUND(o.total_amount_pre_vat * (p.product_manager_commission_rate / 100.0), 0) as process_comm, -- Put in Process/Main column
        0 as stage_comm,
    ...
    WHERE 
        -- Filter Orders
        COALESCE(o.delivery_date, o.created_at)::DATE >= p_start_date 
        AND COALESCE(o.delivery_date, o.created_at)::DATE <= p_end_date
        AND o.status IN ('HoanThanh', 'DaGiaoHang')
        
        -- Filter Profiles (Managers only)
