-- =====================================================
-- Transition Logic: old method ≤ Feb, new method ≥ Mar
...
-- 1b. Helper: Hybrid tier commission (matches frontend logic)
-- FLAT mode: sales within finite-max tier → full sales × rate
-- MARGINAL mode: sales in infinite-max tier → prev tier max × prev rate + excess × current rate
...
BEGIN
    ...
    -- Find active tier (highest tier where sales > min)
    FOR i IN 0..v_count-1 LOOP
        ...
        IF p_sales > v_tier_min THEN
            v_active_idx := i;
            IF v_tier_max > 0 AND p_sales <= v_tier_max THEN
                EXIT; -- Found the tier where sales fits
            END IF;
        END IF;
    END LOOP;
    
    IF v_active_idx < 0 THEN
        RETURN 0;
    END IF;
    
    -- Check if active tier is marginal (max = 0 means infinity)
    ...
    FOR i IN 0..v_count-1 LOOP
        ...
        IF i = v_active_idx THEN
            IF v_is_marginal THEN
                -- Marginal: excess above previous tier's max
                IF i > 0 THEN
                    v_prev_max := COALESCE((p_tiers->(i-1)->>'max')::NUMERIC, 0);
                ELSE
                    v_prev_max := 0;
                END IF;
                v_amount := p_sales - v_prev_max;
            ELSE
                -- Flat: full sales amount
                v_amount := p_sales;
            END IF;
            v_commission := v_commission + (v_amount * v_tier_rate / 100);
        ELSIF v_is_marginal AND i = v_active_idx - 1 THEN
            -- Previous tier in marginal mode: use tier's max
...
-- 2. Staff Commission with transition logic + Manager Revenue Bonus + Company KPI Policy
...
DECLARE
    v_total_revenue_pre_vat NUMERIC := 0;  -- HoanThanh only (for manager bonus)
    v_kpi_revenue NUMERIC := 0;            -- All non-cancelled (for KPI check, matches dashboard)
    v_company_target NUMERIC := 0;
    v_kpi_met BOOLEAN := TRUE;  -- Default: met (if target = 0 or NULL)
...
BEGIN
    ...
    -- 2. Revenue for KPI CHECK (all non-cancelled orders, matches dashboard display)  
    ...
    -- 4. Determine if KPI is met (target=0 → always met)
    ...
    WITH participated_tasks AS (
        ...
        AND p.role != 'QuanLySanXuat'  -- Managers excluded from stage commission
        AND (
    ...
    -- Regular staff commissions (0 if Company KPI not met)
    staff_result AS (
    ...
    -- Manager revenue bonus (100% if KPI met, 40% if not)
    manager_bonus AS (
    ...
    -- Combine: managers get ONLY mgr_bonus (no stage comm), staff get stage comm only
    combined AS (
        SELECT 
...
-- 3. Staff Activity Details with same transition logic
...
-- 4. Sales Commission with correct param signature + transition logic
...
BEGIN
    ...
    SELECT 
        ...
        ms.total_sales as completed_sales, ms.order_count as completed_order_count,
        ms.effective_tiers as commission_tiers,
        -- personal_comm (using helper function with per-month tiers)
        calc_personal_commission(ms.total_sales, ms.effective_tiers) as personal_comm,
        v_group_total_sales as group_sales_total,
        v_group_rate as group_rate,
        v_group_bonus_fund as group_bonus_fund,
        -- group_comm
        (CASE WHEN v_group_total_sales > 0 THEN (ms.total_sales / v_group_total_sales) * v_group_bonus_fund ELSE 0 END) as group_comm,
        -- total_comm
